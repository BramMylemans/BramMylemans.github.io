...
class ChessGame {
    ...
    recursiveAIStart(board) {
        ...
        switch (validMovesLength) {
            case 0:                                                                                     // should never occur - endgame should be captured in main thread
    ...
    getValidMovesFor(board, validMoves, x, y, ai) {         // update validMoves with all moves for a piece at given position
        ...
        function consider(board, x, y, tx, ty, reset, volatile = false, setPieces = []) {
            ...
            if (_this.isCheck(newBoard)) return                                                 // return if move puts king in check
    ...
    evaluateBoard(board) {
        ...
        for (var x = 0; x < 8; ++x) {
            for (var y = 0; y < 8; ++y) {
                ...
                // cheaper alternative: penalty for pieces on last row (excl pawns & kings):
